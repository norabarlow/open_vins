...
namespace ov_msckf {
    ...
    VioManagerOptions parse_command_line_arguments(int argc, char** argv) {
        ...
        // Global gravity
        std::vector<f_ekf> gravity = {params.gravity(0), params.gravity(1), params.gravity(2)};
        ...
        std::vector<std::vector<f_ekf>> p_intrinsic;
        std::vector<std::vector<f_ekf>> p_extrinsic;
        ...
        for(int i=0; i<params.state_options.num_cameras; i++) {
            ...
            Eigen::Matrix<f_ekf,8,1> intrinsics;
            ...
            Eigen::Matrix<f_ekf,7,1> extrinsics;
