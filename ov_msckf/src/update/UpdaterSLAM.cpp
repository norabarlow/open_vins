...
void UpdaterSLAM::delayed_init(std::shared_ptr<State> state, std::vector<std::shared_ptr<Feature>>& feature_vec) {
    ...
    for(const auto &clone_calib : state->_calib_IMUtoCAM) {
        ...
        for(const auto &clone_imu : state->_clones_IMU) {
            ...
            Eigen::Matrix<f_ekf,3,3> R_GtoCi = clone_calib.second->Rot()*clone_imu.second->Rot();
            Eigen::Matrix<f_ekf,3,1> p_CioinG = clone_imu.second->pos() - R_GtoCi.transpose()*clone_calib.second->pos();
    ...
    while(it2 != feature_vec.end()) {
        ...
        Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> H_f;
        Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> H_x;
        Eigen::Matrix<f_ekf,Eigen::Dynamic,1> res;
        ...
        if(feat_rep==LandmarkRepresentation::Representation::ANCHORED_INVERSE_DEPTH_SINGLE) {
            ...
            Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> H_xf = H_x;
        ...
        f_ekf sigma_pix_sq = ((int)feat.featid < state->_options.max_aruco_features)? _options_aruco.sigma_pix_sq : _options_slam.sigma_pix_sq;
        Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> R = sigma_pix_sq*Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic>::Identity(res.rows(), res.rows());

        // Try to initialize, delete new pointer if we failed
        f_ekf chi2_multipler = ((int)feat.featid < state->_options.max_aruco_features)? _options_aruco.chi2_multipler : _options_slam.chi2_multipler;
...
void UpdaterSLAM::update(std::shared_ptr<State> state, std::vector<std::shared_ptr<Feature>>& feature_vec) {
    ...
    Eigen::Matrix<f_ekf,Eigen::Dynamic,1> res_big = Eigen::Matrix<f_ekf,Eigen::Dynamic,1>::Zero(max_meas_size);
    Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> Hx_big = Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic>::Zero(max_meas_size, max_hx_size);
    Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> R_big = Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic>::Identity(max_meas_size,max_meas_size);
    ...
    while(it2 != feature_vec.end()) {
        ...
        /// Chi2 distance check
        Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> P_marg = StateHelper::get_marginal_covariance(state, Hxf_order);
        Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> S = H_xf*P_marg*H_xf.transpose();
        f_ekf sigma_pix_sq = ((int)feat.featid < state->_options.max_aruco_features)? _options_aruco.sigma_pix_sq : _options_slam.sigma_pix_sq;
        S.diagonal() += sigma_pix_sq*Eigen::Matrix<f_ekf,Eigen::Dynamic,1>::Ones(S.rows());
        ...
        if(chi2 > chi2_multipler*chi2_check) {
            if((int)feat.featid < state->_options.max_aruco_features)
                printf(YELLOW "[SLAM-UP]: rejecting aruco tag %d for chi2 thresh (%.3f > %.3f)\n" RESET,(int)feat.featid,double(chi2),double(chi2_multipler*chi2_check));
            (*it2)->to_delete = true;
            it2 = feature_vec.erase(it2);
            continue;
        }
        ...
        if((int)feat.featid < state->_options.max_aruco_features)
            printf("[SLAM-UP]: accepted aruco tag %d for chi2 thresh (%.3f < %.3f)\n",(int)feat.featid,double(chi2),double(chi2_multipler*chi2_check));
...
void UpdaterSLAM::change_anchors(std::shared_ptr<State> state) {
    ...
    f_ts marg_timestep = state->margtimestep();
...
void UpdaterSLAM::perform_anchor_change(std::shared_ptr<State> state, std::shared_ptr<Landmark> landmark, f_ts new_anchor_timestamp, size_t new_cam_id) {
    ...
    Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> H_f_old;
    std::vector<Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic>> H_x_old;
    ...
    Eigen::Matrix<f_ekf,3,3> R_GtoIOLD = state->_clones_IMU.at(old_feat.anchor_clone_timestamp)->Rot();
    Eigen::Matrix<f_ekf,3,3> R_GtoOLD = state->_calib_IMUtoCAM.at(old_feat.anchor_cam_id)->Rot()*R_GtoIOLD;
    Eigen::Matrix<f_ekf,3,1> p_OLDinG = state->_clones_IMU.at(old_feat.anchor_clone_timestamp)->pos()-R_GtoOLD.transpose()*state->_calib_IMUtoCAM.at(old_feat.anchor_cam_id)->pos();

    // NEW: anchor camera position and orientation
    Eigen::Matrix<f_ekf,3,3> R_GtoINEW = state->_clones_IMU.at(new_feat.anchor_clone_timestamp)->Rot();
    Eigen::Matrix<f_ekf,3,3> R_GtoNEW = state->_calib_IMUtoCAM.at(new_feat.anchor_cam_id)->Rot()*R_GtoINEW;
    Eigen::Matrix<f_ekf,3,1> p_NEWinG = state->_clones_IMU.at(new_feat.anchor_clone_timestamp)->pos()-R_GtoNEW.transpose()*state->_calib_IMUtoCAM.at(new_feat.anchor_cam_id)->pos();

    // Calculate transform between the old anchor and new one
    Eigen::Matrix<f_ekf,3,3> R_OLDtoNEW = R_GtoNEW*R_GtoOLD.transpose();
    Eigen::Matrix<f_ekf,3,1> p_OLDinNEW = R_GtoNEW*(p_OLDinG-p_NEWinG);
    ...
    // OLD: anchor camera position and orientation
    Eigen::Matrix<f_ekf,3,3> R_GtoIOLD_fej = state->_clones_IMU.at(old_feat.anchor_clone_timestamp)->Rot_fej();
    Eigen::Matrix<f_ekf,3,3> R_GtoOLD_fej = state->_calib_IMUtoCAM.at(old_feat.anchor_cam_id)->Rot()*R_GtoIOLD_fej;
    Eigen::Matrix<f_ekf,3,1> p_OLDinG_fej = state->_clones_IMU.at(old_feat.anchor_clone_timestamp)->pos_fej()-R_GtoOLD_fej.transpose()*state->_calib_IMUtoCAM.at(old_feat.anchor_cam_id)->pos();

    // NEW: anchor camera position and orientation
    Eigen::Matrix<f_ekf,3,3> R_GtoINEW_fej = state->_clones_IMU.at(new_feat.anchor_clone_timestamp)->Rot_fej();
    Eigen::Matrix<f_ekf,3,3> R_GtoNEW_fej = state->_calib_IMUtoCAM.at(new_feat.anchor_cam_id)->Rot()*R_GtoINEW_fej;
    Eigen::Matrix<f_ekf,3,1> p_NEWinG_fej = state->_clones_IMU.at(new_feat.anchor_clone_timestamp)->pos_fej()-R_GtoNEW_fej.transpose()*state->_calib_IMUtoCAM.at(new_feat.anchor_cam_id)->pos();

    // Calculate transform between the old anchor and new one
    Eigen::Matrix<f_ekf,3,3> R_OLDtoNEW_fej = R_GtoNEW_fej*R_GtoOLD_fej.transpose();
    Eigen::Matrix<f_ekf,3,1> p_OLDinNEW_fej = R_GtoNEW_fej*(p_OLDinG_fej-p_NEWinG_fej);
    new_feat.p_FinA_fej = R_OLDtoNEW_fej*landmark->get_xyz(true)+p_OLDinNEW_fej;

    // Get Jacobians of p_FinG wrt new representation
    Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> H_f_new;
    std::vector<Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic>> H_x_new;
    ...
    Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> Phi = Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic>::Zero(phisize, current_it);
    Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> Q = Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic>::Zero(phisize, phisize);

    // Inverse of our new representation
    // pf_new_error = Hfnew^{-1}*(Hfold*pf_olderror+Hxold*x_olderror-Hxnew*x_newerror)
    Eigen::Matrix<f_ekf,Eigen::Dynamic,Eigen::Dynamic> H_f_new_inv;
    if(phisize==1) {
        H_f_new_inv = f_ekf(1.0)/H_f_new.squaredNorm()*H_f_new.transpose();
    } else {
        H_f_new_inv = H_f_new.colPivHouseholderQr().solve(Eigen::Matrix<f_ekf,3,3>::Identity());
    }
