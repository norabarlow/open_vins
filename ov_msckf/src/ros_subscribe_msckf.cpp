...
int main(int argc, char** argv) {
    ...
    for(const auto &pair : params.stereo_pairs) {
        ...
        auto image_sub0 = std::unique_ptr<message_filters::Subscriber<sensor_msgs::Image>>(new message_filters::Subscriber<sensor_msgs::Image>(nh,cam_topic0,5));
        auto image_sub1 = std::unique_ptr<message_filters::Subscriber<sensor_msgs::Image>>(new message_filters::Subscriber<sensor_msgs::Image>(nh,cam_topic1,5));
        auto sync = std::unique_ptr<message_filters::Synchronizer<sync_pol>>(new message_filters::Synchronizer<sync_pol>(sync_pol(5), *image_sub0,*image_sub1));
