...
namespace ov_core {
    ...
    public:
         ...
         */
        void update_feature(size_t id, float timestamp, size_t cam_id,
         ...
         */
        std::vector<std::shared_ptr<Feature>> features_not_containing_newer(float timestamp, bool remove=false, bool skip_deleted=false) {
         ...
         */
        std::vector<std::shared_ptr<Feature>> features_containing_older(float timestamp, bool remove=false, bool skip_deleted=false) {
         ...
         */
        std::vector<std::shared_ptr<Feature>> features_containing(float timestamp, bool remove=false, bool skip_deleted=false) {
         ...
         */
        void cleanup_measurements(float timestamp) {
            std::unique_lock<std::mutex> lck(mtx);
            for (auto it = features_idlookup.begin(); it != features_idlookup.end();) {
                    ...
                    for(const auto &times : feat.second->timestamps) {
                        ...
                        if(temp->timestamps.find(cam_id)==temp->timestamps.end()) {
                            ...
                            for(size_t i=0; i<feat.second->timestamps.at(cam_id).size(); i++) {
                                float time_to_find = feat.second->timestamps.at(cam_id).at(i);
