...
namespace ov_type {
    ...
    public:
        ...
        IMU() : Type(15) {
            ...
            Eigen::Matrix<f_ekf,Eigen::Dynamic,1> imu0 = Eigen::Matrix<f_ekf,Eigen::Dynamic,1>::Zero(16,1);
            imu0(3) = 1.0;
        ...
        */
        void update(const Eigen::Matrix<f_ekf,Eigen::Dynamic,1>& dx) override {
            ...
            Eigen::Matrix<f_ekf, 16, 1> newX = _value;
        ...
        /// Velocity access
        Eigen::Matrix<f_ekf, 3, 1> vel() const {
            return _v->value();
        }

        // FEJ velocity access
        Eigen::Matrix<f_ekf, 3, 1> vel_fej() const {
            return _v->fej();
        }

        /// Gyro bias access
        Eigen::Matrix<f_ekf, 3, 1> bias_g() const {
            return _bg->value();
        }

        /// FEJ gyro bias access
        Eigen::Matrix<f_ekf, 3, 1> bias_g_fej() const {
            return _bg->fej();
        }

        /// Accel bias access
        Eigen::Matrix<f_ekf, 3, 1> bias_a() const {
            return _ba->value();
        }

        // FEJ accel bias access
        Eigen::Matrix<f_ekf, 3, 1> bias_a_fej() const {
            return _ba->fej();
        }
